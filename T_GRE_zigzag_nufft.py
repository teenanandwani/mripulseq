...
for ii in range(-Nphase//2, Nphase//2): 
    seq.add_block(rf1)
    gx = pp.make_trapezoid(channel='x', area=Nread/2 * np.sin(ii / Nphase * np.pi/180) + 1e-7, duration=1e-3, system=system)
    adc = pp.make_adc(num_samples=Nread, duration=7.5e-3, phase_offset=0 * np.pi / 180, delay=gx.rise_time, system=system)
    
    gx = pp.make_extended_trapezoid(channel = 'x', amplitudes=np.array([0, gx.amplitude, gx.amplitude,0,
                                                                        0, gx.amplitude, gx.amplitude,0,
                                                                        0, gx.amplitude, gx.amplitude,0,
                                                                        0, gx.amplitude, gx.amplitude,0,
                                                                        0]), system=system,
                                    times=np.array([0, 0.17*t, 0.33*t, 0.5*t,
                                                    1.5*t, 1.83*t, 2.17*t, 2.5*t,
                                                    3.5*t, 3.83*t, 4.17*t, 4.5*t,
                                                    5.5*t, 5.83*t, 6.17*t, 6.5*t, 
                                                    7.5*t
                                                    ]))
    
    gy = pp.make_trapezoid(channel='y', area=Nread/2 * np.sin(ii / Nphase * np.pi/180) + 1e-7, duration=1e-3, system=system)
